...
//localStorage.removeItem("soilMoistureChart");
...
function initializeSoilMoistureChart() {
    ...
    var chart1 = new Chart(ctx1, {
      ...
      options: {
        ...
        title: {
          display: true,
          text: "Biểu đồ đường theo thời gian (Độ ẩm Đất)",
        },
        scales: {
          x: {
            display: true,
    ...
    var storedData = localStorage.getItem("soilMoistureChart");
    if (storedData) {
      ...
      var humidityRef = ref(database, '/Sensor/soil_moisture_percent');
  ...
  function createHumidityLineChart() {
    var chart1 = initializeSoilMoistureChart();
  
    // Update the chart data for humidity every second
    function updateSoilMoistureChart() {
      ...
      if (hours === 0 && minutes === 0 && seconds === 0) {
        // Xóa toàn bộ dữ liệu độ ẩm trong thư mục "humidityChartData" của Firebase
        var databaseRef = ref(database, "soilMoistureChart");
        remove(databaseRef);
      }
      ...
      onValue(humidityRef, function (snapshot) {
        ...
        localStorage.setItem("soilMoistureChart", JSON.stringify(chart1.data));
      });
    }
    setInterval(updateSoilMoistureChart, 1000);
  }
  ...
  function exportHumidityDataToCSV() {
    var databaseRef = firebase.database().ref("soilMoistureChart");
    databaseRef.once("value", function (snapshot) {
      var chart1Data = snapshot.val();
      if (chart1Data) {
        var csvContent = "Time,Soil Moisture\n";
        ...
        var fileName = "soilMoistureChart_" + day + "-" + month + "-" + year + ".csv";
